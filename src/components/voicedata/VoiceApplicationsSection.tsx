...
import { voiceApplicationNodes } from "./voiceApplicationsData";
...
export default function VoiceApplicationsSection() {
  return (
      ...
      <Container className="relative z-10">
        <div className="mx-auto max-w-2xl text-center">
          <span className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
            AI Dataset Applications
          </span>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
            One voice dataset practice, ten AI applications
          </h2>
          <p className="mt-3 text-base leading-relaxed text-slate-500">
            The same speech data pipeline trains recognition, synthesis, biometrics, and
            conversational models alike.
          </p>
        </div>

        <div className="relative mx-auto mt-16 hidden aspect-square max-w-[640px] lg:block">
          <div aria-hidden className="absolute inset-[20%] rounded-full border border-slate-100" />

          <svg aria-hidden className="absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 100 100">
            {voiceApplicationNodes.map((node) => {
          ...
          {voiceApplicationNodes.map((node, i) => {
            const pos = nodePosition(node.angle);
            return (
              <motion.div
                key={node.label}
                ...
                style={pos}
              >
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600">
                  <node.icon size={16} strokeWidth={1.75} />
                </span>
                <span className="text-[11px] font-medium leading-tight text-slate-600">{node.label}</span>
              </motion.div>
            );
          })}
        </div>
        ...
        <div className="mt-12 lg:hidden">
          <IconGrid
            items={voiceApplicationNodes.map((node) => ({ icon: node.icon, title: node.label }))}
