...
interface ProcessSectionProps {
  ...
  /** Tailwind grid-cols class for the desktop horizontal layout, e.g. "grid-cols-6" */
  ...
  return (
    <section className="relative w-full overflow-hidden py-16 lg:py-24">
      <Container className="relative z-10">
        ...
        <div className="relative mt-14 lg:hidden">
          ...
          <div className="flex flex-col gap-7">
            {steps.map((step, i) => (
              ...
              >
                <span className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-full border-4 border-white bg-brand-blue-50 text-brand-blue-600 shadow-[0_10px_24px_-8px_rgba(37,99,235,0.35)]">
        ...
        {/* desktop: horizontal timeline with traveling glow-pulse */}
        <div className="relative mt-16 hidden lg:block">
          ...
          <motion.div
            ...
            className="absolute left-0 right-0 top-7 h-px -translate-y-1/2 bg-gradient-to-r from-brand-blue-400 via-cyan-300 to-violet-400"
          />
          <motion.span
            aria-hidden
          ...
          <div className={`grid gap-6 ${desktopColumnsClassName}`}>
            {steps.map((step, i) => (
              ...
              >
                <span className="relative z-10 flex h-14 w-14 items-center justify-center rounded-full border-4 border-white bg-brand-blue-50 text-brand-blue-600 shadow-[0_10px_24px_-8px_rgba(37,99,235,0.35)] transition-transform duration-300 group-hover:-translate-y-1.5 group-hover:scale-105">
                  <step.icon size={22} strokeWidth={1.75} />
                </span>
                <h3 className="mt-4 text-sm font-bold leading-tight text-slate-900">
                  {i + 1}. {step.title}
                </h3>
                <p className="mt-1.5 max-w-[220px] text-xs leading-relaxed text-slate-500">{step.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
